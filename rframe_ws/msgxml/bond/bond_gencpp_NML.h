...
#ifndef _CODEGEN_BOND_GENCPPNML_H_
#define _CODEGEN_BOND_GENCPPNML_H_

#include <messaging/nml/NMLMessageFactory.h>
//#include <DataTypeInfo.h>
#include <messaging/Message.h>

#include "bond_gencpp_NMLSupport.h"
...
namespace bond {
    ...
    class bond_gencppNMLMessageFactory : public NMLMessageFactory
    {
    public:
        /** constructor */
        bond_gencppNMLMessageFactory();
        /** destructor */
        virtual ~bond_gencppNMLMessageFactory();
    ...
    NMLMessageFactory * allocate_bond_gencpp();

    /** NML interface for Constants 

    */
    class ConstantsNML : public NMLmsg
    {
    public:
        ...
        /** assignment operator */
        ConstantsNML & operator=(const ConstantsNML &inobj);
        /** assignment operator for base data class */
        ConstantsNML & operator=(const Constants &inobj);
        ...
        /** nml payload */
        Constants nmlPayload;

        /** set payload */
        void payload(const Constants &data) 
        {
            nmlPayload = data;
        };

        /** set payload from shared_ptr */
        void payload(std::shared_ptr<Constants> & data)
        {
            nmlPayload = *data.get();
        };

        /** \return pointer to nml payload */
        Constants * payload() { return & nmlPayload; };

    //     std::shared_ptr<Constants> nmlPayload; reminder that nmlPayload used to be a pointer
    ...
    /** NML interface for Status 

    */
    class StatusNML : public NMLmsg
    {
    public:
        ...
        /** assignment operator */
        StatusNML & operator=(const StatusNML &inobj);
        /** assignment operator for base data class */
        StatusNML & operator=(const Status &inobj);
        ...
        /** set payload */
        void payload(const Status &data) 
        {
            nmlPayload = data;
        };

        /** set payload from shared_ptr */
        void payload(std::shared_ptr<Status> & data)
        {
            nmlPayload = *data.get();
        };

        /** \return pointer to nml payload */
        Status * payload() { return & nmlPayload; };

    //     std::shared_ptr<Status> nmlPayload; reminder that nmlPayload used to be a pointer
    // but the only way this could work well is if we knew to instantiate it due to dataTypeFlat()
    // which we cannot know until compile/run time...
    };
    ...
    void nmlupdate(CMS *cms, bond::Constants & inobj);
    /** serialize a message to/from a buffer 
     \param cms buffer data structure to be serailized to/from
     \param inobj object to be serialized*/
    void nmlupdate(CMS *cms, bond::Status & inobj);

    /** nml format function */
    int bond_gencpp_format(NMLTYPE type, void * buffer, CMS* cms);

}; // end namespace bond

#endif // _BOND_GENCPPNML_H_
